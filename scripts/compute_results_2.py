"""
scripts/compute_results_2.py

Loads saved model checkpoints and computes final results
across all seeds for all projects.

Usage:
    python scripts/compute_results_2.py --project alluxio --seeds 1 2 3
    python scripts/compute_results_2.py --all --seeds 1 2 3
    python scripts/compute_results_2.py --all --seeds 3 4 5
"""
...
def safe_torch_load(path: Path):
    """
    Loads a torch file safely across different PyTorch versions.
    """
    try:
...
def evaluate_checkpoint(
    ...
    graph = safe_torch_load(graph_path)
    ...
    # ── Load checkpoint ──────────────────────────────────────────
    checkpoint_dir = processed_dir / project / f"seed_{seed}" / "checkpoint"
    ...
    if len(eval_out) == 5:
        y_true, y_pred, h, A_hat, edge_index = eval_out

    elif len(eval_out) == 4:
        y_true, y_pred, h, A_hat = eval_out

        if hasattr(graph, "edge_index"):
            edge_index = graph.edge_index
        else:
            logger.error(
                "trainer.evaluate() returned 4 values, but graph.edge_index "
                "does not exist. Cannot compute refactoring recommendations."
            )
            return None, None

    else:
        raise ValueError(
            f"trainer.evaluate() returned {len(eval_out)} values. "
            "Expected either 4 or 5."
        )

    # Ensure tensors are on CPU
    y_true = y_true.detach().cpu()
    y_pred = y_pred.detach().cpu()
    A_hat = A_hat.detach().cpu() if torch.is_tensor(A_hat) else A_hat
    edge_index = edge_index.detach().cpu() if torch.is_tensor(edge_index) else edge_index
    ...
    test_idx = graph.test_mask.nonzero(as_tuple=True)[0].cpu()
    n_methods = graph.num_nodes

    if not hasattr(graph, "target_class"):
        logger.error("graph.target_class not found.")
        return det_metrics, None

    if not hasattr(graph, "source_class"):
        logger.error("graph.source_class not found.")
        return det_metrics, None

    source_classes = graph.source_class.cpu()
    target_class = graph.target_class.cpu()

    n_classes = int(target_class.max().item()) + 1
    ...
    pred_targets = pred_targets_full[test_idx].cpu()
    true_targets = target_class[test_idx].cpu()
...
def compute_project_results(
    ...
    for seed in seeds:
        ...
        if det_m is None or ref_m is None:
            logger.warning(
                f"Skipping seed {seed} — checkpoint, graph, "
                f"or required fields were not available."
            )
            continue
    ...
    if not det_results:
        logger.error(
            f"No results for {project} — check that checkpoints exist."
        )
        return {}, {}
    ...
    logger.info(
        ...
        f"mean ± std over {len(det_results)} seeds:"
    )
...
def print_summary_table(all_results: dict):
    ...
    for project, res in all_results.items():
        if not res["detection"] or not res["refactoring"]:
...
def main():
    parser = argparse.ArgumentParser(
        description="Compute final results from saved checkpoints"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=256,
        help="Hidden dimension used during training. Default: 256"
    )
